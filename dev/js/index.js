    ...
    if($(window).width() > 695) {
        ...
        $(document).scroll(function(){
            ...
            if(scrollTop > header_h) {
                ...
                appLogo.css({
                    'background': 'url(' + baseUrl + 'assets/icons/logo_' + themeColorFlag + '.svg) no-repeat center',
            ...
            if (scrollTop > 480) {
                $('.catalog-container').addClass('fixed');
            } else{
                $('.catalog-container').removeClass('fixed');                
            }
            scFlag = scrollTop;
        });
    }
        ...
        $.getJSON(baseUrl + 'search.json').done(function(data) {
            ...
            for (var i in data) {
                ...
                if(keywords !== '' && k.toLowerCase().indexOf(keywords) >= 0) {
                    html += '<a class="search_item" href="' + baseUrl.replace(/\/$/, '') + item.url + '">' + item.title + '</a>';
                }
            }
